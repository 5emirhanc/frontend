...
function Cards() {
  ...
  for (let index = 0; index < 1; index++) {
    axios.get("http://localhost:5000/products/file").then((response) => {
      ...
      localStorage.setItem("res", response.data);
      console.log(localStorage.getItem('res').length,'uzunluk');
    });
  }
  ...
  return(
    <div>
      ...
      {(() => {
        ...
        for (let i = 0; i <= localStorage.getItem('res').length; i++) {
          options.push(<Card>
            <Card.Img
              className="rsm"
              variant="top"
            ...
            />
            <Card.Body>
              <Card.Title>{data[i].fileName}</Card.Title>
              <Card.Text>
