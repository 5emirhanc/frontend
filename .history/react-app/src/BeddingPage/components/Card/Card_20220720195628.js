  ...
  const getProducts = async () => {
    ...
    console.log(
      window.URL.createObjectURL(
        new Blob(response.data[0].ProductImage.data[0], { type: "buffer" })
      ),
      "image"
    );
    setImage(
      Buffer.from(response.data[0].ProductImage.data[1], "binary").toString(
        "base64"
      )
    );
  };
        ...
        <tbody>
          {products.map((product) => (
            <tr>
              ...
              <td>
                <img width={60} src={product.imgs}></img>
              </td>
